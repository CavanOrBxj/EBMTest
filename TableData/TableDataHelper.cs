    ...
    public class TableDataHelper
    {
        ...
        public static bool WriteTable(TableType type, params object[] data)
        {
            try
            {
                ...
                if (type == TableType.Content)
                {
                    if (data!=null)
                    {

                        BindingCollection<EBMTest.EBMContent.EBContent_AllData> EBContent_List_AllData = (BindingCollection < EBMTest.EBMContent.EBContent_AllData >) data[0];

                        JObject jo = new JObject();

                        List<EBMTest.EBMContent.EBContentTMP> List0 = new List<EBMContent.EBContentTMP>();
                        List<EBMTest.EBMContent.EBContent> List1 = new List<EBMContent.EBContent>();


                        foreach (var item in EBContent_List_AllData)
                        {
                            EBMTest.EBMContent.EBContentTMP tmp = new EBMContent.EBContentTMP();
                            tmp.EBM_ID = item.EBM_ID;
                            tmp.Guid = item.Guid;
                            tmp.SectionName = item.SectionName;
                            tmp.SendState = item.SendState;
                            List0.Add(tmp);
                            if (item.EBContentList!=null)
                            {
                                List1.AddRange(item.EBContentList);
                            }
                        }
                        jo["0"] = JsonConvert.SerializeObject(List0);
                        jo["1"] = JsonConvert.SerializeObject(List1);
                        File.WriteAllText(path, jo.ToString(), Encoding.UTF8);
                    }
                }
                else
