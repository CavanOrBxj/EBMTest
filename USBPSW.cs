...
namespace EBMTest
{
    public class USBE
    {
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "OpenDevice", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        public static extern int OpenDevice(ref System.IntPtr phDeviceHandle);

        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "CloseDevice", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        public static extern int CloseDevice(ref int phDeviceHandle);

        //导入证书
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "ImportTrustedCert", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        public static extern int ImportTrustedCert(ref int phDeviceHandle, StringBuilder strcertPath);

        //使用设备私钥计算数据签名
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "GenerateSignatureWithDevicePrivateKey", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        ...
        //使用设备私钥计算数据签名(字符串模式)
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "GenerateSignatureWithDevicePrivateKey_String", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        ...
        //使用证书验证数据签名
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "VerifySignatureWithTrustedCert", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        ...
        //使用证书验证数据签名（字符串）
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "VerifySignatureWithTrustedCert_String", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        public static extern int VerifySignatureWithTrustedCert_String(ref int phDeviceHandle, int nDataType, byte[] pucData);

        //计算数据摘要
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "CalcHash", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        ...
        //生成签名
        [DllImport("libTassYJGBCmd_SJJ1507.dll", EntryPoint = "Platform_CalculateSignature", CharSet = CharSet.Ansi, CallingConvention = CallingConvention.Cdecl, ExactSpelling = true)]
        ...
        public int USB_OpenDevice(ref System.IntPtr phDeviceHandle)
        {
            try
            {
                ...
                {
                    //Ping ping = new Ping();
                    //PingReply reply = ping.Send("www.baidu.com",2);//百度IP
        ...
        //public int Platform_CalculateSingature_int(int PhDeviceHandle, int DataType, byte[] strpcData, int size, ref byte[] strSingBody)
        //{
        //    string CalculateSignaturestr = string.Empty;
        //    try
        //    {
        //        strSingBody = new byte[74];
        //        byte[] pucCounter = new byte[100]; //注意 返回的签名数据是100个字节 定长
        //        int pucSignCerSn = 100;
        //        int nResule = Platform_CalculateSignature(ref PhDeviceHandle, DataType, strpcData, size, pucCounter, ref pucSignCerSn);
        //            byte[] bytescal=new byte[74];
              
        //        //输出签名结果格式为 Base64  直接转str
        //        CalculateSignaturestr = Encoding.Default.GetString(pucCounter);  //把base64格式 的byte[] 转string;
        //        bytescal = Convert.FromBase64String(CalculateSignaturestr);
        //        strSingBody = bytescal;
