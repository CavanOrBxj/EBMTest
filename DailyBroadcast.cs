...
namespace EBMTest
{
    public partial class DailyBroadcast : Form
    {
        ...
        private void DelDaily()
        {
            if (dgvTotal.SelectedRows.Count > 0)
            {
                byte cmdTag = (dgvTotal.SelectedRows[0].DataBoundItem as DailyProgram).B_Daily_cmd_tag;
                TotalConfig_List.RemoveAt(dgvTotal.SelectedRows[0].Index);
                switch (cmdTag)
                {
                    case ChangeProgramTag:
                        dgvChangeProgram.DataSource = null;
                        break;
                    case OutSwitchTag:
                        dgvOutSwitch.DataSource = null;
                        break;
                    case PlayCtrlTag:
                        dgvPlayCtrl.DataSource = null;
                        break;
                    case RdsTransferTag:
                        dgvRdsTransfer.DataSource = null;
                        break;
                }
            }
            else
            ...
            public override string Summary
            {
                get
                {
                    return string.Join("/", B_Switch_status, Utils.ComboBoxHelper.GetTypeStringValue(Enums.ParamType.AddressType, B_Address_type),
                        Program.list_Terminal_Address.Count > 0 ? Program.list_Terminal_Address[0] + "..." : "...");
                }
            }
        }
            ...
            public override string Summary
            {
                get
                {
                    return string.Join("/", Volume, Utils.ComboBoxHelper.GetTypeStringValue(Enums.ParamType.AddressType, B_Address_type),
                        Program.list_Terminal_Address.Count > 1 ? Program.list_Terminal_Address[0] + "..." : "...");
                }
            }
        }
