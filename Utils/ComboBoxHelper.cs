...
namespace EBMTest.Utils
{
    public class ComboBoxHelper
    {
        ...
        public static void InitConfigRebackType(ComboBox box)
        {
            ...
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "IP通道，使用UDP协议，固定服务器ip地址回传";
            dr["Value"] = 2;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "IP通道，使用UDP协议，域名服务器地址回传";
            dr["Value"] = 3;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "IP通道，使用TCP协议，固定服务器ip地址回传";
            dr["Value"] = 4;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "IP通道，使用TCP协议，域名服务器地址回传";
            dr["Value"] = 5;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "GPRS通道，使用UDP协议，固定服务器ip地址回传";
            dr["Value"] = 6;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "GPRS通道，使用UDP协议，域名服务器地址回传";
            dr["Value"] = 7;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "GPRS通道，使用TCP协议，固定服务器ip地址回传";
            dr["Value"] = 8;
            dt.Rows.Add(dr);
            dr = dt.NewRow();
            dr["Display"] = "GPRS通道，使用TCP协议，域名服务器地址回传";
            dr["Value"] = 9;
            dt.Rows.Add(dr);
            box.DisplayMember = "Display";
            box.ValueMember = "Value";
