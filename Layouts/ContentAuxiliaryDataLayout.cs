...
namespace EBMTest.Layouts
{
    public partial class ContentAuxiliaryDataLayout : UserControl
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (bindList.Count == 1)
            {
                if (!((bindList[0].Type >= 1 && bindList[0].Type <= 2) && (type >= 41 && type <= 45)) &&
                        !((bindList[0].Type >= 41 && bindList[0].Type <= 45) && (type >= 1 && type <= 2)))
                {
            ...
            if (!string.IsNullOrWhiteSpace(textAuxiliaryData.Text) && cbBoxType.SelectedIndex >= 0)
            {
                ...
                {
                    DisplayData = textAuxiliaryData.Text.Trim(),//Path.GetFileName(textAuxiliaryData.Text.Trim()),
                    Type = type,
                });
            }
        }
