...
namespace EBMTest
{
    public partial class EBMConfigure : Form
    {
                    ...
                    if (d.SendState)
                    {
                        switch (d.B_Daily_cmd_tag)
                        {
                            ...
                            case Utils.ComboBoxHelper.ConfigureSetAddressTag:
                                if(SingletonInfo.GetInstance().IsProtocolGX)
                                {
                                 cmd.Add((d as SetAddress).Configure.GetCmdGX());
                                }
                                else
                                {
                            ...
                            case Utils.ComboBoxHelper.ConfigureWorkModeTag:
                                ...
                                if(SingletonInfo.GetInstance().IsProtocolGX)
                                {
                                    cmd.Add((d as WorkMode).Configure.GetCmdGX());
                                }
                                else
                                {
                            ...
                            case Utils.ComboBoxHelper.ConfigureMainFrequencyTag:
                                ...
                                if (SingletonInfo.GetInstance().IsProtocolGX)
                                {
                                    cmd.Add((d as MainFrequency).Configure.GetCmdGX());
                                }
                                else
                                {
                            ...
                            case Utils.ComboBoxHelper.ConfigureDefaltVolumeTag:
                                ...
                                if(SingletonInfo.GetInstance().IsProtocolGX)
                                {
                                  cmd.Add((d as DefaltVolume).Configure.GetCmdGX());
                                }
                                else
                                {
                            ...
                            case Utils.ComboBoxHelper.ConfigureRebackPeriodTag:
                                if(SingletonInfo.GetInstance().IsProtocolGX)
                                {
                                  cmd.Add((d as RebackPeriod).Configure.GetCmdGX());
                                }
                                else
                                {
        ...
        public class Reback : Configure
        {
            ...
            public EBConfigureRebackGX Configure { get; set; }//修改于20180316 广西版本 增加了地址类型 及当批类型时的主备地址
            public byte B_Address_type
            {
                get { return Configure.B_Address_type; }
                set { Configure.B_Address_type = value; }
            ...
            public string S_reback_address_backup
            {
                get { return Configure.S_reback_address_backup; }
                set { Configure.S_reback_address_backup = value; }
            }

            public int I_reback_port
            {
                get { return Configure.I_reback_port; }
                set { Configure.I_reback_port = value; }
            }

            public int I_reback_port_Backup
            {
                get { return Configure.I_reback_port_Backup; }
                set { Configure.I_reback_port_Backup = value; }
            }
            ...
            public override string Summary
            {
                get
                {
                    if (B_reback_type==(byte)2)
                    {
                        return string.Join("/", Utils.ComboBoxHelper.GetTypeStringValue(Enums.ParamType.ConfigRebackType, B_reback_type), S_reback_address, I_reback_port.ToString(),S_reback_address_backup,I_reback_port_Backup.ToString(),
                            Utils.ComboBoxHelper.GetTypeStringValue(Enums.ParamType.AddressType, B_Address_type), "...");
                    }
                    else
                    {
