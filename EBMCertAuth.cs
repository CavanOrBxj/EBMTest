...
namespace EBMTest
{
    public partial class EBMCertAuth : Form
    {
        ...
        private EBCertAuthTable datatmp;
        ...
        /// <param name="e"></param>
        private void btnManuallySend_Click(object sender, EventArgs e)
        {
            try
            {
                if ((MdiParent as EBMMain).IsStartStream)
                {
                    btnManuallySend.Enabled = false;


                    if (GetSendCertAuth().Count > 0)
                    {
                        ...
                        (MdiParent as EBMMain).EbMStream.EB_CertAuth_Table.Repeat_times = 1;
                        (MdiParent as EBMMain).UpdateDataText();
                    }
                    btnManuallySend.Enabled = true;
                }
