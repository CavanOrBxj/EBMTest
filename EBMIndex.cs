...
namespace EBMTest
{
    public partial class EBMIndex : Form
    {
        ...
        public event EventHandler<EbmIdChangedEventArgs> EbmIdChanged;
        ...
        private void dgvEBIndex_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex >= 0 && e.RowIndex >= 0)
            {
                if (e.ColumnIndex == ColumnSendState.Index)
                {
                    ...
                    //if (EBIndex_List[e.RowIndex].SendState)
                    //{
                    //    (MdiParent as EBMMain).InitStreamTable();
                    //}
                }
        ...
        private void OnEbmIdChanged()
        {
            if (EbmIdChanged != null) EbmIdChanged(this, new EbmIdChangedEventArgs(GetEbmId()));
        }

        public List<string> GetEbmId()
        {
            List<string> ebmlist = new List<string>();
            foreach (var index in EBIndex_List)
            {
                ebmlist.Add(index.S_EBM_id);
            }
            return ebmlist;
        }
